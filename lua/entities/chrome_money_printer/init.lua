...
function ENT:Initialize()
	...
	self:SetColor(Color(0, 255, 51, 0))
...
function ENT:CreateMoneybag()
	...
	local Y = GAMEMODE.Config.chromeprintamount
	if amount == 0 then
		amount = 10
	end
...
function ENT:Use(activator)
	...
	DarkRP.notify(activator, 1, 4, "You have collected $"..self:GetNWInt("PrintA").." from a Chrome Printer.")
	self:SetNWInt("PrintA",0)
	end
end
